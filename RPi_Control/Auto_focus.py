...
import cv2, numpy as np, time, subprocess as sp
...
# Autofocus routine
def autofocus(stride = 50):
    max_sharpness = 0
    best_position = 0

    for i in range(0, 1000, stride):  # Limit the number of steps

        path_name = f"z_image2500/z-pos_{i}.jpg"
        iso = 100
        cmd = ["libcamera-still", "-o", path_name, "--nopreview"]
